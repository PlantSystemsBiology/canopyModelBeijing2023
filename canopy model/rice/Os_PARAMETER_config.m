...
STEP_Y = 25; % column distance, y direction  列距离，单位cm，这个是输入参数，用户可调节。
ROW_NUM = 3; % row number of canopy 行数，即构建几行水稻，这个是输入参数，用户可调节。
COL_NUM = 6; % col number 列数。这个是输入参数，用户可调节。
