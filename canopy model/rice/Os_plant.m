...
M_plant = zeros(0,OS_DATA_MATRIX_COLUMN_NUM); % empty single plant data matrix，一株水稻的数据。
tillerNum = max(paramMatrix(:,2)); % total tiller number.检查输入数据有多少个分蘖，因为输入数据文件的第1列为植株ID，第2列为分蘖ID。

for tillerID = 1:tillerNum % 按照分蘖（tiller）的顺序来依次构建每一个分蘖。
   % tillerID
    ...
    X = X + X_move_outter;  % 分蘖基部位置向外平移距离
